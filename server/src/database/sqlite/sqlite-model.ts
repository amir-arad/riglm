...
/**
 * SQLite model class
 * Provides MongoDB-compatible API for SQLite collections
 */
export class SQLiteModel<T extends IDocument> implements ModelProvider<T> {
  private collection: ReturnType<
    typeof SQLiteDatabase.prototype.getCollection<T>
  >;
  ...
  constructor(
    ...
    this.schema = schema;
    this.collection = SQLiteDatabase.getInstance().getCollection<T>(entityName);
  ...
  async create(data: Partial<T>): Promise<T> {
    const id =
      data._id || SQLiteDatabase.getInstance().generateId(this.entityName);
    const now = new Date();
